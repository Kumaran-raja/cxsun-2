...
const HeroBanner2: React.FC<HomeBannerProps> = ({
  ...
  return (
    <div className="relative w-full h-screen md:h-[90vh] overflow-hidden">
      ...
      <div className={`absolute inset-0 ${bgType==='dark'?"bg-black/70":"bg-white/30"}`} />

      {/* Content Layer */}
      <div className="absolute inset-0 flex items-center px-8 md:px-20">
        <div className={`max-w-xl ${bgType==='dark'?"text-white":"text-black"} space-y-4`}>
          <p className="text-lg font-medium">{year}</p>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight">
            {title}
          </h1>
          <p className={`text-base md:text-lg ${bgType==='dark'?"text-white":"text-black"}`}>{description}</p>
          ...
          <div className="relative w-full max-w-4xl mx-4">
            ...
            <div className="aspect-video w-full bg-black rounded-lg overflow-hidden shadow-lg">
              <iframe
                src={videoPath}
                title="Video Player"
                aria-label="Video player showing Everest Hotel introduction"
