  ...
  return (
    <div className="rounded-xl">
      ...
      <div className="flex gap-4 flex-col pb-3 ">
        ...
        <div
          ref={menuref}
          className="flex flex-wrap justify-center gap-3 border border-ring/30 rounded-lg bg-primary/10 p-3 w-full max-w-max mx-auto shadow-2xl"
        >
          {items.map((tab, index) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              style={{ animationDelay: `${index * 0.3}s` }} // stagger delay
              className={`px-4 py-2 rounded-md text-sm font-medium transition cursor-pointer animate__animated ${
                ...
                activeTab === tab.id
                  ...
                  : "bg-background border border-ring/30 hover:bg-highlight1"
              }`}
            >
              {tab.label}
            </button>
      ...
      {activeContent && (
        <div
          ref={cardref}
          className="mt-8 md:mt-15 grid md:grid-cols-2 gap-8 items-center"
        >
          {/* Left Text Content */}
          <div
            className={`order-2 md:order-1 animate__animated ${inView2 ? "animate__fadeInLeft" : "opacity-0"}`}
          >
          ...
          {/* Right Image */}
          <div
            className={`order-1 md:order-2 animate__animated ${inView2 ? "animate__fadeInRight" : "opacity-0"}`}
          >
            <img
